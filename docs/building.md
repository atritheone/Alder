...
Run `npm run installer` to build the self-contained Windows x64 application and an offline installer under `release/Alder-0.12.0-Windows-x64/` (using the package version). The installer and its `.7z` data file must stay together. The separate data file avoids the embedded payload size limit with Alder's large speech resources. Setup verifies SHA-256 before extracting, installs for the current Windows user, creates a Start menu shortcut and registers an uninstaller with Windows Settings. It requires Windows 10 or later. No files are downloaded during installation.
